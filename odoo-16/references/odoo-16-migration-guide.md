...
description: Comprehensive guide for upgrading modules and data to Odoo 16,
...
Comprehensive guide for migrating modules and data to Odoo 16, covering migration scripts, upgrade hooks, deprecations, and best practices.
...
5. [Deprecations in Odoo 16](#deprecations-in-odoo-16)
...
```python
# File: migrations/16.0.1.0/pre-migrate_data.py

def migrate(cr, version):
    """
    Migration script for Odoo 16.0
...
^(6.1|6.0-16.0|saas~11-99)\.?  # Optional server version prefix
...
16.0       # Odoo 16.0, module version 0
```
...
```python
# File: migrations/16.0.1.0/pre-update_schema.py
...
```python
# File: migrations/16.0.1.0/post-migrate_data.py
...
```python
# File: migrations/16.0.1.0/end-update_references.py
...
Odoo 16 provides a tool for automated source code transformations:

```bash
# Upgrade from one version to another
./odoo-bin upgrade_code --from 17.0 --to 16.0

# Run specific upgrade script
./odoo-bin upgrade_code --script 17.5-01-tree-to-list

# Upgrade specific addons
./odoo-bin upgrade_code --addons-path=/path/to/addons --from 17.0 --to 16.0
```

### Upgrade Script Template

```python
# File: odoo/upgrade_code/16.0-01-example.py
...
```python
# File: migrations/16.0.1.0/tests/test_migration.py
...
```python
# File: migrations/16.0.1.0/post-migrate_currency.py
...
```python
# File: migrations/16.0.1.0/post-migrate_m2m.py
...
v2 = parse_version('16.0.0.1')

if v2 > v1:
    print("v2 is newer")

# Supports semantic versioning
parse_version('16.0.1.0.alpha')  # Pre-release
parse_version('16.0.1.0.beta1')   # Beta
parse_version('16.0.1.0.rc2')     # Release candidate
```
...
def adapt_version(version):
    ...
    serie = release.major_version  # e.g., "16.0"
...
# Examples:
adapt_version('1.0')      # → '16.0.1.0'
adapt_version('16.0.1.0') # → '16.0.1.0'
```
...
def migrate(cr, version):
    ...
    # Migrate from 17.0
    if parsed_version < parse_version('16.0'):
