...
```python
# GOOD: Batch create (Odoo 16 standard)
records = self.create([
...
### Use cr.dictfetchall() for Dict Results (Odoo 16)

```python
# GOOD: parameterized query, returns list of dicts
...
## Flush & Recompute (Odoo 16)

### Understanding Flush

Odoo 16 uses lazy writes - changes are cached and flushed to database later.
...
### Flush Before SQL Query (Odoo 16)
